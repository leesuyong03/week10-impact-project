...
```mermaid
graph TD
    A[Step 1. 영상 촬영<br/>스마트폰 고속촬영] --> B[Step 2. 영상 로드<br/>및 캘리브레이션]
    B --> C[Step 3. 과일 궤적 추적<br/>Point Tracking]
    C --> D[Step 4. 속도<br/>데이터 추출]
...
#### 촬영 팁
- 손떨림을 최소화하기 위해 삼각대 사용 필수
- 영상이 과일의 충돌 전후 과정을 모두 포함하도록 조정
...
#### 영상 불러오기
- Tracker 실행 후 촬영한 영상 파일 드래그 앤 드롭

#### 영상 클립 자르기
- 타임라인 하단의 검은색 화살표를 드래그하여 분석 구간 설정
- 분석 구간: 과일이 손에서 떨어지는 시점 ~ 바닥에 부딪힌 후 가장 높이 튀어오른 시점
...
#### Point Mass 생성
- `Track` $\rightarrow$ `New` $\rightarrow$ `Point Mass` 선택
...
#### 속도 그래프 활성화
- 우측 그래프의 y축 라벨을 클릭하여 `y-velocity v_y`로 변경
...
- 과일 질량($m$) 측정 (저울 이용)
- 뉴턴 제2법칙 적용: 
$$F_{avg} = m \cdot \frac{v_2 - v_1}{\Delta t}$$
*(부호에 주의하여 속도 변화량 계산)*
...
### 📊 비교 분석 항목
| 항목 | 맨바닥 (Hard Surface) | 완충재 (Soft Surface) |
|------|----------------------|----------------------|
| 충돌 직전 속도 $v_1$ (m/s) | | |
| 충돌 직후 속도 $v_2$ (m/s) | | |
| 반발 계수 $e$ | | |
...
**Q1. 슬로우 모션 촬영 프레임 레이트는 높을수록 좋은가?**
> A: 네, 높을수록 충돌 순간을 더 자세히 포착할 수 있습니다. 최소 120fps 권장.

**Q2. 캘리브레이션 오류가 크게 나온다면?**
> A: 줄자의 두 지점이 정확히 선택되었는지, 그리고 줄자가 카메라 렌즈와 평행한지 확인하세요.

**Q3. 과일이 손상되어 반발 계수가 0에 가까우면?**
> A: 이는 정상적인 결과입니다. 완충재 없이 낙하할 경우 과일 손상으로 인해 반발력이 크게 감소할 수 있습니다.
